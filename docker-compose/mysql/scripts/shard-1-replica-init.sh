...
until mysql -h url-db-shard-1-master -u root -p"$MYSQL_ROOT_PASSWORD" -e "SELECT 1" &>/dev/null; do
  echo "Waiting for master..."
  sleep 2
done

echo "Configuring replication for shard 1..."

# Check binary logging
mysql -h url-db-shard-1-master -u root -p"$MYSQL_ROOT_PASSWORD" -e "SHOW VARIABLES LIKE 'log_bin';"

# Get master status (MySQL 8+ syntax)
MASTER_STATUS=$(mysql -h url-db-shard-1-master -u root -p"$MYSQL_ROOT_PASSWORD" -e "SHOW BINARY LOG STATUS" --skip-column-names --raw 2>/dev/null)
...
CHANGE REPLICATION SOURCE TO
  SOURCE_HOST='url-db-shard-1-master',
...
echo "Replication for shard 1 configured successfully."